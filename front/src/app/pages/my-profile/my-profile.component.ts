...
export class MyProfileComponent implements OnInit {
  ...
  getUser() {
    this.apiSubscription = this.apiSvc.getUserMe().subscribe(
      (result: UserModel) => {
      ...
      err => {
        this.globals.showToast(
          "Oh não!",
          "Falha ao consultar os dados de seu usuário",
          "error"
        );
      }
    );
  ...
  openModalImageCropPicker() {
    const modalRef = this.modalService.open(ImageCropPickerComponent, {
      size: "lg"
    });
    modalRef.componentInstance.image = this.user.photo ? this.user.photo : null;
    modalRef.result
      .then((result: any) => {
        if (result) this.user.photo = result;
        this.onSubmit();
      })
      .catch(err => {
